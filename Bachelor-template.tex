% !Mode:: "TeX:UTF-8"
%% 请使用 XeLaTeX 编译本文.
 \documentclass{NJUPTThesis}% 选项 forprint: 交付打印时添加, 避免彩色链接字迹打印偏淡. 即使用下一行:
 %\documentclass[forprint]{NJUPTThesis}

\begin{document}
%%%%%%% 下面的内容, 据实填空.

\title{毕\ 业\ 设\ 计（论\ 文）}
\ThesisTitle{南京邮电大学本科毕业论文\LaTeX~模板}
\Major{测试专业名}                  % 专业中文名
\author{测试名}                            % 作者名字
\StudentNumber{Q13010499}            %班级学号
\Supervisor{测试名\quad 教授}        %指导教师中文名、职称
\SchoolName{南京邮电大学测试用学院名}          % 指导单位
\date{XXXX年XX月XX日 \ 至 \ XXXX年XX月XX日}                    % 日期。
...
本文档下载更新地址: \url{https://github.com/liborui/NJUPTThesis}. 使用之前, 请移步查看是否有更新.

问题反馈及建议, 请联系: librchn@gmail.com.
